...
adv_n_bp = Blueprint("adivinhar_numero",__name__, template_folder= 'templates')
...
@adv_n_bp.route('', methods=['GET', 'POST'])
def adivinhar_numero():
    ...
    if 'numero_secreto_an' not in session or request.form.get('novo_jogo_an'):
        ...
        if request.form.get('novo_jogo_an'):
            return redirect(url_for('adivinhar_numero.adivinhar_numero')) # Redireciona para limpar o POST
    ...
    if request.method == 'POST' and not session.get('jogo_an_terminado'):
        if 'palpite_usuario' in request.form:
            ...
            return redirect(url_for('adivinhar_numero.adivinhar_numero')) # Redireciona para GET
